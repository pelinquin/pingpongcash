...
def message2(node, sc, rid, m, pp):
    "_"
    db, k, dat = dbm.open('cles'), ecdsa(), datencode()
...
def buy2(node, sc, rid, pc, cry, pp, mes=''):
    "gui call"
    prc, src, db, k, dat, cry = float(pc), b64tob(bytes(sc, 'ascii')), dbm.open('cles'), ecdsa(), datencode(), _cur[cry]
...
def postig2(node, sc, p1, pf, xi, url, pp):
    "_"
    db, k, src = dbm.open('cles'), ecdsa(), b64tob(bytes(sc, 'ascii'))
...
def debt(d, cm, cry=b'A'):
    ...
    if cm in dc and len(cm) == 9 and len(dc[cm]) == 144:
        root, k = dc[b'_'], ecdsa()
